  ...
  useEffect(() => {
    try {
      ...
      if (connected === "true") {
        ...
        setFitness(result);
        localStorage.setItem("health:fitness", JSON.stringify(result));
      } else if (connected === "false") {
        setHealthConnected(false);
        setFitness(null);
        localStorage.setItem("health:fitness", "N/A");
      } else {
          ...
          <div className="mt-2 rounded-2xl border border-card-border bg-card p-4">
            {healthConnected === true ? (
              <div>
                ...
                {fitness && (
                  <p className="mt-1 text-xs text-muted">Score: {fitness.score}</p>
                )}
              </div>
            ) : healthConnected === false ? (
              <p className="text-sm text-muted">N/A (not connected)</p>
            ) : (
              <p className="text-sm text-muted">Sign in to choose connection</p>
            )}
          </div>
        </div>
